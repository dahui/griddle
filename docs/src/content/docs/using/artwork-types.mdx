...
**Size −/+** above the results makes the tiles bigger or smaller, and the grid re-flows to fit.
Each tab remembers its own setting, so heroes can stay large while capsules stay dense, and the
choice is kept between sessions.

It is a pair of buttons rather than a slider on purpose: a slider cannot be driven with a
controller, because left and right are how you move between things.

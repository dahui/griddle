...
/**
 * The way out. First in the sticky toolbar, and the first thing the pad reaches going up.
 *
 * Section `toolbar`, shared with the zoom steppers at the other end of the same bar — they are
 * one visual row, so left and right should walk along it. A section per control would make the
 * two ends separate rows and put the steppers a *vertical* move away from something beside them.
 */
export function BackButton({ onClick }: { onClick: () => void }) {
  const { ref, focused } = useFocusItem<HTMLButtonElement>('toolbar', 0, 0);
  return (
